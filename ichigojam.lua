...
function new_std15 (screen_w, screen_h, buff_w, buff_h)
   local o = {}
   o.buff_w = buff_w
   o.buff_h = buff_h
   ...
   o.draw_char = function (self, cx, cy, c)
	 ...
	 local line
	 if y < 4 then
	    line = math.floor(hi_bits / 2^((CHAR_H-y-1-4)*CHAR_W)) % 0x100
	 else
	    line = math.floor(lo_bits / 2^((CHAR_H-y-1)*CHAR_W)) % 0x100
	 end
	 for x = 0, CHAR_W-1 do
	    if (math.floor(line / 2^(CHAR_W-x-1)) % 0x2) == 0x1 then
	       local x0 = (cx * CHAR_W + x) * self.dot_w
	       local y0 = (cy * CHAR_H + y) * self.dot_h
	       love.graphics.rectangle("fill", x0, y0, self.dot_w, self.dot_h)
	    end
	 end
      end
   end

   o.draw_screen = function (self)
      for y = 0, self.buff_h-1 do
	 for x = 0, self.buff_w-1 do
